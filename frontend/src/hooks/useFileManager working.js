...
export default function useFileManager() {
  ...
  const fetchingFilesRef = useRef(false);
const fetchingFoldersRef = useRef(false);
  ...
  const fetchFiles = useCallback(async (pageNumber,folder_id, isInitial = false) => {
    fetchingfiles = true
    // setLoading(true);
    ...
    // Append files if infinite scrolling down, replace if new folder/search/filter
    if (pageNumber === 1 || isInitial) {
...
const loadMore = useCallback(() => {
  if (!fetchingFilesRef.current && hasMore && !isFetchingMore) {
    setCurrentPage((prev) => prev + 1);
  }
}, [hasMore, isFetchingMore]);

// useEffect(() => {
//   fetchFiles(currentPage, currentFolderId, currentPage === 1);
// }, [currentPage, fetchFiles]);
  ...
  useEffect(() => {
  if (currentPage > 1) {
    fetchFiles(currentPage, currentFolderIdRef.current, false);
  }
}, [currentPage, fetchFiles]);
