...
function App() {
  ...
  // Listens for the session-expired signal from utils/api.js's response
  // interceptor and does a normal client-side redirect instead of the
  // window.location.href hard reload that used to live there.
  useEffect(() => {
    const onSessionExpired = () => {
      logout();
      navigate('/login', { replace: true });
    };
    window.addEventListener('sfms:session-expired', onSessionExpired);
    return () => window.removeEventListener('sfms:session-expired', onSessionExpired);
  }, [logout, navigate]);
