...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      if (token) {
        ...
        if (decodedUser) {
          api.defaults.headers.common['Authorization'] = `Bearer ${token}`
          if (isMounted) setUser(decodedUser)
  ...
  // 2. Periodic background revalidation & cross-tab sync
  useEffect(() => {
    if (!user) return
  ...
  }, [user?.id, logout]) // Depend on primitive ID to avoid unnecessary interval re-creations
