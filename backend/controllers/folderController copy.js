...
/**
 * togglePinFolder — Toggles the pinned state of a folder.
 * PUT /folders/:folderId/pin
 */
const togglePinFolder = async (req, res) => {
  try {
    const { folderId } = req.params;
    const userId = req.user.id;
    const isAdmin = req.user.role === 'admin';
    ...
    if (!isAdmin && folder.created_by !== userId) {
      return res.status(403).json({ error: 'Not authorized to pin this folder' });
    }

    const updated = await pool.query(
      `UPDATE virtual_folders SET is_pinned = NOT is_pinned WHERE folder_id = $1 RETURNING *`,
      [folderId]
    );
    ...
    console.error('Toggle folder pin error:', err);
    res.status(500).json({ error: 'Failed to toggle folder pin state' });
  }
};
