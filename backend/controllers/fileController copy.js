...
const os = require('os');
const HTMLtoDOCX = require('html-to-docx'); // NEW — npm install html-to-docx
...
const MAX_EDITABLE_DOCX_SIZE = 8 * 1024 * 1024; // 8 MB — docx has more overhead (styles, media)
...
// Word documents (.docx / OOXML only — legacy binary .doc isn't supported by
// mammoth). Edited as HTML in the browser: mammoth.convertToHtml() renders
// the docx for editing, html-to-docx converts the edited HTML back to a
// real .docx on save.
...
const getFileContent = async (req, res) => {
  try {
    ...
    if (isDocxFile(file)) {
      if (stat.size > MAX_EDITABLE_DOCX_SIZE) {
        return res.status(413).json({ error: `File is too large to edit online (max ${MAX_EDITABLE_DOCX_SIZE / (1024 * 1024)} MB).` });
      }
      const { value: html } = await mammoth.convertToHtml({ path: fullPath });
      return res.json({ type: 'docx', html, fileName: file.file_name, mimeType: file.mime_type, size: stat.size });
    }

    if (!isEditableTextFile(file)) {
      return res.status(415).json({ error: 'This file type cannot be opened in the editor.' });
    }
    ...
    res.json({ type: 'text', content, fileName: file.file_name, mimeType: file.mime_type, size: stat.size });
  } catch (err) {
...
const updateFileContent = async (req, res) => {
  try {
    ...
    // ── Word document (.docx) save path ──────────────────────────────
    if (isDocxFile(file)) {
      if (typeof html !== 'string') return res.status(400).json({ error: 'html must be a string.' });
      if (Buffer.byteLength(html, 'utf8') > MAX_EDITABLE_DOCX_SIZE) {
        return res.status(413).json({ error: `Content is too large to save (max ${MAX_EDITABLE_DOCX_SIZE / (1024 * 1024)} MB).` });
      }
      ...
      try {
        ...
        console.error('HTML→DOCX conversion error:', conversionErr);
        return res.status(500).json({ error: 'Failed to convert content back to a Word document.' });
      }

      await fsPromises.writeFile(fullPath, docxBuffer);
      await pool.query('UPDATE files SET file_size = $1 WHERE id = $2', [docxBuffer.length, fileId]);
      await logAction({
        req, action: 'file.edit', targetType: 'file', targetId: fileId, targetLabel: file.file_name,
        metadata: { via: 'inline-editor-docx' },
      });
      return res.json({ success: true, size: docxBuffer.length, updatedAt: new Date().toISOString() });
    }

    // ── Plain text / code file save path ─────────────────────────────
...
const getFileActivity = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, action, target_type, target_id, target_label, metadata, status, actor_user_id, actor_user_id, created_at
       FROM audit_logs
       WHERE target_id = $1 
         AND target_type = $2
...
const splitPdf = async (req, res) => {
  try {
    ...
    // Stateless — matches toolsController.js's PDF endpoints: stream the
    // result straight back to the browser as a download. Nothing is
    // written to disk and nothing is inserted into the files table, since
    // this is a one-off extract, not a new stored file.
    const ext         = path.extname(file.file_name);
    const baseName    = path.basename(file.file_name, ext);
    const downloadName = `${baseName}_pages${fromPage}-${toPage}${ext || '.pdf'}`;

    await logAction({
      req, action: 'file.pdf_split', targetType: 'file', targetId: file.id, targetLabel: file.file_name,
      metadata: { fromPage, toPage, mode: 'download-only' },
    });

    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(downloadName)}"`);
    res.setHeader('Content-Length', newBytes.length);
...
const copyFiles = async (req, res) => {
  ...
  [
    ...
    file.description, target_folder_id, file.content_raw, file.file_hash, file.thumbnail, // ← added
  ]
);
